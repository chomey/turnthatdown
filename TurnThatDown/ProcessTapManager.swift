...
final class ProcessTapManager: ObservableObject {
    ...
    func setEQGain(_ gain: Float, band: Int, for bundleID: String) {
        guard let index = tappedApps.firstIndex(where: { $0.id == bundleID }) else { return }
        tappedApps[index].eqBands[band].gain = max(-24, min(24, gain))
            ...
            for i in 0..<outBufs.count {
                ...
                if volume > 1.0 || (tap.eq.enabled && !tap.eq.isFlat) {
                    for j in 0..<totalSamples {
                        let x = outF[j]
                        if x > clipThreshold { outF[j] = clipThreshold - 1.0 / (x - clipThreshold + 1.0) }
                        else if x < -clipThreshold { outF[j] = -clipThreshold + 1.0 / (-x - clipThreshold + 1.0) }
                    }
                }
            }
        }
